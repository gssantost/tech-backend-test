...
class User {
  ...
  async createUser({name, middlename, lastname, email, address, art, cinema, music}) {
    try {
      const obj = await db.connect();
      const data = await obj.task(async t => {
        const userExists = await t.oneOrNone(queries.CHECK_USER_BY_EMAIL, email);
        if (userExists === null) {
          const data = await t.any(queries.CREATE_USER, [name, middlename, lastname, email, address]);
